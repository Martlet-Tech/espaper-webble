...
const unsigned char PSR_V[2] = {0xDF, 0x69}; // 0x09 -> 0x69 20240319 V1 update.
...
const unsigned char SPIM_V[1] = {0x10}; // 0 - Single SPI (Default) 1 - Quad SPI
...
void checkBusyHigh(void) // If BUSYN=0 then waiting
{
	...
	while (!(getGpioLevel(EPD_BUSY))) {
		...
		if (cnt >= 100) {
			cnt = 0;
			ESP_LOGE(TAG, "Wait for busy high");
		}
	};
}
...
void initEPD(void)
{
	...
	EPD_IO_WriteCommandData_2CH(AN_TM, AN_TM_V, sizeof(AN_TM_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(CMD66, CMD66_V, sizeof(CMD66_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(PSR, PSR_V, sizeof(PSR_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(CDI, CDI_V, sizeof(CDI_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(TCON, TCON_V, sizeof(TCON_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(AGID, AGID_V, sizeof(AGID_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(PWS, PWS_V, sizeof(PWS_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(CCSET, CCSET_V, sizeof(CCSET_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(TRES, TRES_V, sizeof(TRES_V), CS_MASK_MASTER_SLAVE);
	EPD_IO_WriteCommandData_2CH(PWR, PWR_V, sizeof(PWR_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(EN_BUF, EN_BUF_V, sizeof(EN_BUF_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(BTST_P, BTST_P_V, sizeof(BTST_P_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(BOOST_VDDP_EN, BOOST_VDDP_EN_V, sizeof(BOOST_VDDP_EN_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(BTST_N, BTST_N_V, sizeof(BTST_N_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(BUCK_BOOST_VDDN, BUCK_BOOST_VDDN_V, sizeof(BUCK_BOOST_VDDN_V), CS_MASK_MASTER);
	EPD_IO_WriteCommandData_2CH(TFT_VCOM_POWER, TFT_VCOM_POWER_V, sizeof(TFT_VCOM_POWER_V), CS_MASK_MASTER);
}
...
void EL133UF1_Update(void)
{
	...
	delayms(30);
	EPD_IO_WriteCommandData_2CH(DRF, DRF_V, sizeof(DRF_V), CS_MASK_MASTER_SLAVE);

	checkBusyHigh();
	// ATTENTION: check busy 原本放在CS拉高之前。此为同一般SPI接口屏幕的区别，需要测试是否兼容。
	EPD_IO_WriteCommandData_2CH(POF, POF_V, sizeof(POF_V), CS_MASK_MASTER_SLAVE);
	delayms(1000);
}
...
void EL133UF1_DisplayFrame(const unsigned char *frame_buffer_m, const unsigned char *frame_buffer_s)
{
	...
	ESP_LOGI(TAG, "EL133UF1_DisplayFrame Ready.");
	EL133UF1_Update();
}
